...
notes () {cd /Volumes/GoogleDrive/Meine\ Ablage/;vim -c 'NERDTree .' -c 'normal O'}
tmux-2 () {tmux new \; split-window -h \; split-window -v \; set-option -g mouse on \; attach}
tmux-3 () {tmux new \; split-window -h \; split-window -v \; split-window -v \; set-option -g mouse on \; attach}
